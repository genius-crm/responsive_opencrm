...
class CloudSync_List_View extends Vtiger_Index_View	{
	...
	function __construct() {
		// get showLogs value
                $logs = isset($_REQUEST['showLogs']) ? $_REQUEST['showLogs'] : false;
                // redirect to settings page if showLogs value not set
                if($logs === false && $_REQUEST['_pjax'] != true)       {
	...
	public function initializeListViewContents(Vtiger_Request $request, Vtiger_Viewer $viewer) {
		...
		// removing edit button from mass actions 
		if(isset($linkModels['LISTVIEWMASSACTION']))	{
			foreach($linkModels['LISTVIEWMASSACTION'] as $single_key => $singleLink)	{
				if($singleLink->linklabel == 'LBL_EDIT')
					unset($linkModels['LISTVIEWMASSACTION'][$single_key]);
		...
		if($this->listViewLinks)	{
                        foreach($this->listViewLinks['LISTVIEWBASIC'] as $single_basic_link => $singleBasicLink)        {
                                if($singleBasicLink->linklabel == 'LBL_ADD_RECORD')
                                        unset($this->listViewLinks['LISTVIEWBASIC'][$single_basic_link]);
