...
{if {$REFERENCE_LIST_COUNT} gt 1}
	...
	{/if}
{/if}
<input name="{$FIELD_MODEL->getFieldName()}" id="{$FIELD_MODEL->getFieldName()}" type="hidden" value="{$FIELD_MODEL->get('fieldvalue')}" class="sourceField" data-displayvalue='{$FIELD_MODEL->getEditViewDisplayValue($FIELD_MODEL->get('fieldvalue'))}' data-fieldinfo='{$FIELD_INFO}' />
{assign var="displayId" value=$FIELD_MODEL->get('fieldvalue')}
...
{if $smarty.request.view eq 'Edit' && !in_array($REFERENCE_LIST[0],$QUICKCREATE_RESTRICTED_MODULES) && $FIELD_NAME neq parent_emailcampaignsid}
...
{/if}
{if $FIELD_NAME neq parent_emailcampaignsid}
<span class="add-on clearReferenceSelection cursorPointer">
	<i id="apply_list" class="icon-refresh apply_list" title="{vtranslate('LBL_REFRESH', $MODULE)}" ></i>
</span>
{/if}
</div>
{/strip}
