			...
			<div class="row-fluid contentHeader">
                            ...
                            <div class="span6">
                                <div class="pull-right extensionDetailActions">
                                    {if ($MODULE_ACTION eq 'Installed') and ($REGISTRATION_STATUS) and ($PASSWORD_STATUS)}
                                        <button class="btn btn-danger" type="button" style="margin-right: 6px;" id="uninstallModule"><strong>{vtranslate('LBL_UNINSTALL', $QUALIFIED_MODULE)}</strong></button>
                                    {elseif ($REGISTRATION_STATUS) and ($PASSWORD_STATUS)}
                                        {if $EXTENSION_DETAIL->get('isprotected') && $IS_PRO && ($EXTENSION_DETAIL->get('price') gt 0) && ($CUSTOMER_PROFILE['CustomerCardId'])}
                                            <button class="btn btn-success" type="button" id="installExtension"><strong>{vtranslate($MODULE_ACTION, $QUALIFIED_MODULE)}</strong></button>
                                        {elseif (!$EXTENSION_DETAIL->get('isprotected')) && ($EXTENSION_DETAIL->get('price') gt 0) && ($CUSTOMER_PROFILE['CustomerCardId'])}
                                            <button class="btn btn-success" type="button" id="installExtension"><strong>{vtranslate($MODULE_ACTION, $QUALIFIED_MODULE)}</strong></button>
