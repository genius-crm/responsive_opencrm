    ...
    function vtlib_handler($moduleName, $eventType) {
        if ($eventType == 'module.postinstall') {
            ...
            $adb->pquery("INSERT INTO vtiger_settings_field(fieldid, blockid, name, iconpath, description, linkto, sequence, active) 
                        VALUES(?,?,?,?,?,?,?,?)", array($fieldid, $blockid, 'VGS Hide Banners', '', 'VGSHideAds', 'index.php?module=VGSHideAds&view=IndexVGSHideAds&parent=Settings', $sequence++, 0));
        } else if ($eventType == 'module.disabled') {
            
        } else if ($eventType == 'module.preuninstall') {
            $adb = PearDatabase::getInstance();
            $adb->pquery("DELETE FROM vtiger_settings_field WHERE name=?", array('VGS Hide Banners'));
