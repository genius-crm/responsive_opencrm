...
if(jQuery) (function($){
	...
	$.extend($.fn, {
		fileTree: function(options, file) {
			...
			// storing option script value for future use
                        options.defaultScript = options.script;

                        $(this).each( function() {

                                function showTree(element, dir) {
                                        // adding root value at the end of the url
                                        if(options.script !== undefined)
                                                options.script = options.defaultScript+'&root='+dir;
					...
					.done(function(data){
						...
						$('#loadinggif_modalheader_settings').hide();
					})
					.fail(function(){
						$(element).find('.start').html('');
						$(element).removeClass('wait').append("<li>Unable to get information. Please try again</li>");
						$('#loadinggif_modalheader').hide();
						$('#loadinggif_modalheader_settings').hide();
					});
				}
