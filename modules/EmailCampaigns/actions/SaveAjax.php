...
class EmailCampaigns_SaveAjax_Action extends Vtiger_SaveAjax_Action {
	...
	public function filter_subscribers(Vtiger_Request $request) {
		...
		if($parent_emailcampaignsid == ''){
		    ...
		    $related_list = '';
		    for ($i = 0; $i < $count; $i++) {
			    $related_list1 .= $related_list.''.$db->query_result($result, $i, 'crmid');
				    $related_list = ',';
		    }
		}else{
			  ...
			  $db = PearDatabase::getInstance();
			  if($emailclicklink == '1' && $whopenemail != '1'){
			      ...
			      where vtiger_link_tracker.click > 0 AND vtiger_emailcamp_sendemail.emailcampaignsid = ?';
			  }else{
			      $query = 'select vtiger_emailcamp_sendemail.related_id from vtiger_emailcamp_sendemail
			      INNER JOIN vtiger_email_track ON vtiger_email_track.mailid = vtiger_emailcamp_sendemail.activityid
			      where vtiger_email_track.access_count > 0 AND vtiger_emailcamp_sendemail.emailcampaignsid = ?';
			  }
			  $result = $db->pquery($query, array($parent_emailcampaignsid));
			  $count = $db->num_rows($result);
			  $related_list = '';
			  for ($i = 0; $i < $count; $i++) {
			  ...
			  $result_modulename = $db->pquery($query_modulename, array($parent_emailcampaignsid));
			  $module_name = $db->query_result($result_modulename, 0, 'module_name');
		      }
		      $response->setResult(array('count' =>$count,'related_recorts' =>$related_list1,'module_name' =>$module_name));
		      $response->emit();
	...
	global $current_user;
		...
		$replyTo = $recordModel->get('reply_to');
		...
		$response = new Vtiger_Response();
		$response->setResult(array('success' => true, 'status'=>$mailer));
		$response->emit();
	}
	...
	public function schedulelatersave(Vtiger_Request $request) {
		...
		$next_trigger_time = Vtiger_Datetime_UIType::getDBDateTimeValue($startdate." ".$startTime);
		...
		if ($db->num_rows($result)) {
				...
				$query = "UPDATE vtiger_scheduleemailcampagine SET next_trigger_time = ? ,status=? WHERE emailcampaignsid = ? LIMIT 1";
				$result = $db->pquery($query, array($next_trigger_time,'1',$record));
				$response->setResult(array('success' => true));
		}else{
			$query = "INSERT INTO vtiger_scheduleemailcampagine(`emailcampaignsid`, `status`, `next_trigger_time`) VALUES (?, ?,?)";
			$result = $db->pquery($query, array($record,'1',$next_trigger_time));
	...
	public function sendenow(Vtiger_Request $request) {
		...
		$recordModel = $this->record->getRecord();
		$rajkiran = $this->massSave($recordModel);
		$response = new Vtiger_Response();
		$response->setResult(array('success' => true, '$rajkiran' =>$rajkiran));
		$response->emit();
	}
	
	public function massSave($recordModel_email) {
		...
		$related_recorts = $recordModel_email->get('related_recorts');
		$setindex = 0;		
		$related_recorts_array = explode(',', $related_recorts);
		foreach ($related_recorts_array as $key => $related_recorts_id) {
			$record_exists = $adb->query("SELECT * FROM vtiger_crmentity WHERE deleted = 0 and crmid = $related_recorts_id");
			$count = $adb->num_rows($record_exists);
			if($count > 0){
				$related_moduleName  =  $recordModel_email->get('module_name');
				$related_recordModel = Vtiger_Record_Model::getInstanceById($related_recorts_id, $related_moduleName);
				$emailoptout = $related_recordModel->get('emailcampaign_optout');

				if($emailoptout == 0){
					if($related_moduleName == 'Accounts'){
						$related_recortslist[$setindex + 1] = array("related_recorts_id"=>$related_recorts_id,"email"=>$related_recordModel->get('email1'));	
					}else{
						$related_recortslist[$setindex + 1] = array("related_recorts_id"=>$related_recorts_id,"email"=>$related_recordModel->get('email'));		 
					}
					$setindex++;
				}
			}
		...
		$result1 = $adb->pquery("SELECT* FROM vtiger_scheduleemailcampagine WHERE emailcampaignsid = ?", array($recordId));
		$noOfScheduledReports = $adb->num_rows($result1);
		if($noOfScheduledReports == 0){
			$query1 = "INSERT INTO `vtiger_scheduleemailcampagine` (`emailcampaignsid`, `status`, `next_trigger_time`, `send_email`) VALUES ('$recordId', 0, '$currentTimestamp', 0)";
			$adb->pquery($query1, array());
		}
		 
		$result50 = $adb->pquery("SELECT* FROM vtiger_scheduleemailcampagine WHERE emailcampaignsid = ?", array($recordId));
		$next_trigger_time = $adb->query_result($result50, 0, 'next_trigger_time');	
		$perm_rows = sizeof($related_recortslist);
		if(trim($perm_rows) < trim($batch) && $currentTimestamp > $next_trigger_time){
			 $perm_rows1 = sizeof($related_recortslist);
			 $adb->pquery('UPDATE  vtiger_scheduleemailcampagine SET status=0,batchrecord=? WHERE emailcampaignsid=?', array($batch,$recordId));
			$start_s = 1;
		}else{
			
			$result2 = $adb->pquery("SELECT* FROM vtiger_scheduleemailcampagine WHERE emailcampaignsid = ?", array($recordId));
			$number_send_email = $adb->query_result($result2, 0, 'send_email');
			$batchrecord = $adb->query_result($result2, 0, 'batchrecord');
			$status = $adb->query_result($result2, 0, 'status');

			$start_s = $number_send_email + 1;
			$emailcamp_interval = $recordModel_email->get('emailcamp_interval');
			$emailcamp_interval_minute = '+'.trim($emailcamp_interval).' minutes';
			
			$next_trigger_time = $adb->query_result($result2, 0, 'next_trigger_time');
			$newtimestamp = strtotime($emailcamp_interval_minute, strtotime($next_trigger_time));
			$newtimeinterval = date('Y-m-d H:i:s', $newtimestamp);
			
			$perm_rows2 = $number_send_email + $batch;
			if(trim($perm_rows) < trim($number_send_email) ){
				$perm_rows1 = 0;
				$adb->pquery('UPDATE  vtiger_scheduleemailcampagine SET status=0 WHERE emailcampaignsid=?', array($recordId));
			}else if (trim($perm_rows) == trim($number_send_email)){
				$start_s = 0;
				$perm_rows1 = 0;
				$adb->pquery('UPDATE  vtiger_scheduleemailcampagine SET status=0 WHERE emailcampaignsid=?', array($recordId));
			
			}else if ($currentTimestamp < $next_trigger_time){
				$perm_rows1 = 0;
				$start_s = 0;
			}else{
				
				if($batchrecord > 0){
					$result33 = $adb->pquery("SELECT vtiger_crmentity.createdtime FROM vtiger_emailcamp_sendemail 
					INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_emailcamp_sendemail.activityid 
					WHERE  vtiger_emailcamp_sendemail.emailcampaignsid = ? order by activityid DESC LIMIT 1", array($recordId));
					$last_send_email_time = $adb->query_result($result33, 0, 'createdtime');
				
					if($currentTimestamp > $last_send_email_time){
						$to_time = strtotime($currentTimestamp);
						$from_time = strtotime($last_send_email_time);
						$timediffrent = round(abs($to_time - $from_time) / 60,2);
					}else{
						$timediffrent = 0;
					}
				}else{
				...
				if($batchrecord == 0){
					$perm_rows1 = trim($perm_rows2);
					$adb->pquery('UPDATE  vtiger_scheduleemailcampagine SET status=1,next_trigger_time=?,batchrecord=? WHERE emailcampaignsid=?', array($newtimeinterval,$batch,$recordId));
			
				}elseif($batchrecord == $number_send_email){
					$newbatchrecord = $batchrecord + $batch;
					$adb->pquery('UPDATE  vtiger_scheduleemailcampagine SET status=1,next_trigger_time=?,batchrecord=? WHERE emailcampaignsid=?', array($newtimeinterval,$newbatchrecord,$recordId));
					$perm_rows1 = trim($newbatchrecord);
				}elseif($status == 1 && $number_send_email < $batchrecord && $timediffrent > 30){
					$newtimestamp1 = strtotime($emailcamp_interval_minute, strtotime($currentTimestamp));
					$newtimeinterval1 = date('Y-m-d H:i:s', $newtimestamp1);
					
					$adb->pquery('UPDATE  vtiger_scheduleemailcampagine SET status=1,next_trigger_time=? WHERE emailcampaignsid=?', array($newtimeinterval1,$recordId));
					$perm_rows1 = $batchrecord;
					$start_s = $number_send_email + 1;
				}else{
					$perm_rows1 = 0;
					$start_s = 0;
		...
		if($start_s != 0){
			for ($i = $start_s; $i <= $perm_rows1; $i++) {
				$par_crmid = $related_recortslist[$i]['related_recorts_id']; 

				if($par_crmid!=''){
					$result55 = $adb->pquery("SELECT* FROM vtiger_emailcamp_sendemail WHERE related_id = ? and emailcampaignsid=? ", array($par_crmid,$recordId));
					$count_result55 = $adb->num_rows($result55);
					if($count_result55==0){
						$to = $related_recortslist[$i]['email'];
						...
						$recordModel->set('email_flag', 'SAVED');
						...
						$recordModel->save();
						$record_saveemail_id = $recordModel->getId();
						if($record_saveemail_id !='' && $par_crmid != 0 && $record_saveemail_id != 0 && $par_crmid != ''){
							$query1 = "INSERT INTO `vtiger_emailcamp_sendemail` (`emailcampaignsid`, `related_id`, `activityid`) VALUES ('$recordId', '$par_crmid', '$record_saveemail_id')";
							$result = $adb->pquery($query1, array());
							$adb->pquery('UPDATE  vtiger_scheduleemailcampagine SET send_email= send_email + 1  WHERE emailcampaignsid=?', array($recordId));
							...
							$EmailTrackingReport->set('parent_id', $par_crmid);
							$EmailTrackingReport->set('emailid', $record_saveemail_id);
							$EmailTrackingReport->set('emailcampaignsid', $recordId);
							$EmailTrackingReport->save();
							$emailtrackingreportid = $EmailTrackingReport->getId();	
						}
						
						$status =  EmailCampaigns_SaveAjax_Action::send($recordModel,$recordModel_email,$par_crmid,$emailtrackingreportid);
						if($status === true){
	...
	public	function send($recordModel,$recordModel_email,$par_crmid,$emailtrackingreportid) {
		$unsubscribe_option = $recordModel_email->get('unsubscribe_option');
		$par_module_name = $recordModel_email->get('module_name');
		global $current_user;
		$currentUserModel = Users_Record_Model::getCurrentUserModel();
		...
		$mailer = Emails_Mailer_Model::getInstance();
		$mailer->IsHTML(true);
		$fromEmail = $recordModel_email->get('from_email');
		$replyTo = $recordModel_email->get('reply_to');
		$userName = $recordModel_email->get('from_name');
		...
		if(preg_match_all("/$regexp/siU", $description, $matches, PREG_SET_ORDER))
			{
				...
				{
					$link_url = urlencode($match[2]);
					$link_name=strip_tags($match[3]);
					$src="href='".$siteURL."modules/EmailCampaigns/linktrack.php?current_user_id=$current_user_id&emailtrackingreportid=$emailtrackingreportid&parent_id=$par_crmid&emailid=$id&link_name=$link_name&url=$link_url'";
			...
			if($unsubscribe_option != 0){	
			    ...
			    $unsubscriber="<br><br>".$lblunsubphrase."<a href='".$siteURL."modules/EmailCampaigns/unsubscriber.php?current_user_id=$current_user_id&recordid=$par_crmid&modulename=$par_module_name'>".$lblunsubphraselink."</a>";
			    $description .= $unsubscriber;
			    
			    $subscriber="<br><br>".$lblsubphrase."<a href='".$siteURL."modules/EmailCampaigns/subscriber.php?current_user_id=$current_user_id&recordid=$par_crmid&modulename=$par_module_name'>".$lblsubphraselink."</a>";
			    $description .= $subscriber;			
			}	
		$unsubscriber="<br><br><a href='".$siteURL."modules/EmailCampaigns/unsubscriber.php?current_user_id=$current_user_id&recordid=$par_crmid&modulename=$par_module_name'>".$unsub."</a>";
		$description .= $unsubscriber;
		
		$subscriber="<br><br><a href='".$siteURL."modules/EmailCampaigns/subscriber.php?current_user_id=$current_user_id&recordid=$par_crmid&modulename=$par_module_name'>".$sub."</a>";
		$description .= $subscriber;			
