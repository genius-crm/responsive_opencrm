                    ...
                    <div>
                        ...
                        <div class="extensionInfo">
                            <div class="row-fluid">
                                ...
                                <div class="span5"><span class="rating" data-score="{$ON_RATINGS}" data-readonly=true></span><span>&nbsp;({$EXTENSION->get('avgrating')})</span></div>
                                <div class="span7">
                                    <div class="pull-right">
                                        ...
                                        {if $EXTENSION->isVtigerCompatible()}
                                            {if ($EXTENSION->isAlreadyExists()) and (!$EXTENSION_MODULE_MODEL->get('trial'))}
                                                {if ($EXTENSION->isUpgradable()) and ($REGISTRATION_STATUS) and ($PASSWORD_STATUS)}
                                                ...
                                                {/if}
                                            {elseif (($EXTENSION->get('price') eq 'Free') or ($EXTENSION->get('price') eq 0)) and ($REGISTRATION_STATUS) and ($PASSWORD_STATUS)}
                                                {if $EXTENSION->get('isprotected') && $IS_PRO}
                                                    <button class="oneclickInstallFree btn btn-success">{vtranslate('LBL_INSTALL', $QUALIFIED_MODULE)}</button>
                                                {elseif !$EXTENSION->get('isprotected')}
                                                    <button class="oneclickInstallFree btn btn-success">{vtranslate('LBL_INSTALL', $QUALIFIED_MODULE)}</button>
                                                {/if}
                                            {elseif ($REGISTRATION_STATUS) and ($PASSWORD_STATUS) and ($IS_PRO)}
                                                {if ($EXTENSION->get('trialdays') gt 0) and ($EXTENSION_MODULE_MODEL eq 'false') and ($EXTENSION->get('isprotected') eq 1)}
                                                    <button class="oneclickInstallPaid btn btn-success" data-trial=true>{vtranslate('LBL_TRY_IT', $QUALIFIED_MODULE)}</button>
                                                ...
                                                {/if}
                                                 <button class="oneclickInstallPaid btn btn-info" data-trial=false>{vtranslate('LBL_BUY',$QUALIFIED_MODULE)}${$EXTENSION->get('price')}</button>
                                            {/if}
                                        {else}
