...
class PDFMaker_PDFMaker_Model extends Vtiger_Module_Model {
    ...
    public function GetPreparedMPDF(&$mpdf, $record, $module, $language) {
        ...
        $compositeCount = substr_count($body_html, '$PRODUCTCOMPOSITEDISCOUNT$');
	 	 for ($i = 1; $i <= $compositeCount; $i++) {
			 $start = strpos($body_html, '$PRODUCTCOMPOSITEDISCOUNT$-')+strlen('$PRODUCTCOMPOSITEDISCOUNT$-');
			 $end = strpos($body_html, '-$END$', $start);
			 $line = substr($body_html, $start, $end-$start);
			 if ($line == '$PRODUCTPOSITION$') { $lineId = 0; } else { $lineId = $line; }
			 $query= "select composite from vtiger_discount where entity_id = ? and line_id = ?;";
			 $result = $this->db->pquery($query, array($record, $lineId));
			 $row = $this->db->query_result($result);
			 $body_html = str_replace('$PRODUCTCOMPOSITEDISCOUNT$-'.$line.'-$END$', '('.$row.')&nbsp;&nbsp;&nbsp;', $body_html);
    ...
    public function GetProductBlockFields() {
        ...
        $Product_Fields['PRODUCTCOMPOSITEDISCOUNT$-$PRODUCTPOSITION$-$END'] = vtranslate("LBL_DISCOUNT", "Discount");
