...
class ChartsWidget_MassInactive_Action extends Vtiger_Mass_Action {
	...
	public function process(Vtiger_Request $request) {
		...
		foreach($recordIds as $recordId) {
			...
			if ($row) {
				$query= "update vtiger_chartswidget set active = 0 where chart_id = ?;";
			} else {
				$query= "insert into vtiger_chartswidget (chart_id,active) values (?, 0);";
			}
		...
		$response->setResult(array(vtranslate('LBL_CW_INACTIVE_SUCCESSFULLY', $parentModule)));
