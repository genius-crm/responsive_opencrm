...
{strip}
	...
	<td>
		...
		<span class="stockAlert" style="color: green;">
			{vtranslate('Current state storage','Accounting')}: <span class="maxQuantity">{$data.$qtyInStock}</span>
		</span>
		{/if}
	</td>
	<td>
	...
	<td>
		...
		<div id="discountTotal{$row_no}" align="right" class="discountTotal" style="display:none;">{if $data.$discountTotal}{$data.$discountTotal}{else}0.00{/if}</div>
		<div id="totalAfterDiscount{$row_no}" align="right" class="totalAfterDiscount" style="display:none;">{if $data.$totalAfterDiscount}{$data.$totalAfterDiscount}{else}0.00{/if}</div>
		<div id="taxTotal{$row_no}" align="right" class="productTaxTotal {if $IS_GROUP_TAX_TYPE}hide{/if}" style="display:none;">{if $data.$taxTotal}{$data.$taxTotal}{else}0.00{/if}</div>
	</td>
	<td>
