 ...
 * All Rights Reserved.
 *************************************************************************************/
jQuery.Class("SubscribersList_Popup_Js",{
	...
	destroy : function(){
		...
		//window.close();
		//var subscribers_list_display = 'subscribers_list_display';
//window.opener.document.forms['EmailCampaigns_step2'].subscribers_list_display.value == 'rajkiran';
//window.opener.document.EmailCampaigns_step2
		//jQuery.triggerParentEvent(eventToTrigger, JSON.stringify(result));
//window.opener.jQuery("#subscribers_list_display").value == 'rajkiran';
	...
	getListViewEntries: function(e){
		...
		if(typeof dataUrl != 'undefined'){
		    ...
		    response[id] = {'name' : recordName,'info' : recordInfo} ;
				...
				var sourcefield = this.getSourceField();
				window.opener.document.getElementById(sourcefield+"_display").value = recordName;
				window.opener.document.getElementById(sourcefield).value = id;
          // window.opener.EmailCampaigns_Edit2_Js.registerReferenceSelectionEvent();
			window = self;
